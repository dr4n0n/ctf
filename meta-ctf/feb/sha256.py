...
try:
    ...
    print("user_shared=", user_shared)
    shared = pow(g, secret, p)
    print("shared=", shared)

    print(
        "sha256(str(user_shared).encode()).digest()=",
        sha256(str(user_shared).encode()).digest(),
    )
    print()
    print(
        "sha256(str(shared).encode()).digest()=", sha256(str(shared).encode()).digest()
    )

    if (
        sha256(str(user_shared).encode()).digest()
        == sha256(str(shared).encode()).digest()
    ):
